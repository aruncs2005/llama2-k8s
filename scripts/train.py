...
def main():
    ...
    #     model_inputs = tokenizer(examples["text"],return_token_type_ids=False)
    ...
    #model = LlamaForCausalLM.from_pretrained(args.model_name_or_path,cache_dir=args.cache_dir,token=args.access_token,torch_dtype=torch.bfloat16)
    model.config.use_cache = False # this is needed for falcon

    num_params = compute_num_params(model)
    if is_main_process(args.rank):
        logger.info(f"Total number of parameters {num_params}")

    falcon_auto_wrap_policy = functools.partial(
        transformer_auto_wrap_policy,
        transformer_layer_cls={
            get_module_class_from_name(model,"LlamaDecoderLayer"),
        },
    )
    ...
    model = FSDP(
        model,
        auto_wrap_policy=falcon_auto_wrap_policy,
    ...
    check_fn_gpt = lambda submodule: isinstance(submodule, get_module_class_from_name(model,"LlamaDecoderLayer"))
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if is_main_process(args.rank):
            logger.info(f"*******{epoch=}: {eval_ppl=} {eval_loss=}*******")
